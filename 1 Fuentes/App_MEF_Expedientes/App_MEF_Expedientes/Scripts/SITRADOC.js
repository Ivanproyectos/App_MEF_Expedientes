...
SITRADOC = {
    ...
    MultiplicaNumeros: function (ListaNumeros, decimal) {
        ...
        $.each(ListaNumeros, function (k, Numero) {
            ...
            if (posCaracterDecimal > 0) {
                ...
                numeroEntero = parseInt(parteEntera) * parseInt("1" + SITRADOC.PadLeft("", parteDecima.toString().length, "0")) + parseInt(parteDecima);
        ...
        if (Producto.toString().length > cantDecimal) {
            ...
            resultado = "0." + SITRADOC.PadLeft(Producto.toString(), dif, "0");
        }
        if (decimal != null) {
    ...
    GetTimeToDate: function (date) {
        ...
        if (date != null && date != "") {
            if (date.indexOf("/Date(") >= 0) {
                ...
                var fecharecepcion = SITRADOC.getDate(fecha) + '/' + SITRADOC.getMonth(fecha) + '/' + fecha.getFullYear();
    ...
    Grilla: function (grilla, pager, identificador, height, width, caption, urlListar, id, colsNames, colsModel, sortName, opciones) {
        ...
        if (!opciones.GridLocal) {
                ...
                subGridRowExpanded: function (subgrid_id, row_id) {
                    ...
                    $.ajax({
                        ...
                        success: function (rsp) {
                            var data = (typeof rsp.d) == 'string' ? ('(' + rsp.d + ')') : rsp.d;
                ...
                gridComplete: function () {
                    if ($('#' + grilla).getGridParam('records') == 0) {
                        if (opciones.noregistro == true) {
                            SITRADOC.ShowAlert("dialog-alert", "Sin Registro");
                        }
                    }
            ...
            $('#' + grilla).jqGrid({
                ...
                groupingView: {
                    ...
                    groupText: ['<span style="background-image:none;background-color:#f4f3f3;display:block;margin-right:-2px;margin-left:-2px;padding:4px;font-weight:bold"> {0} - {1} Expediente(s) </span>'],
                ...
                onClickButton: function () {
                    if (rowKey != null) {
                        if (opciones.btnEliminar == null) {
                            SITRADOC.confirm(opciones.Mensaje, function () { Eliminar(); }, '', opciones.dialogDelete, opciones.Lenguaje);
                        }
                        else {
                            if (opciones.btnEliminar.Mensaje != null && opciones.btnEliminar.Mensaje != '') {
                                SITRADOC.confirm(opciones.btnEliminar.Mensaje, function () { opciones.btnEliminar.Function(rowKey); }, '', opciones.btnEliminar.titleMensaje, opciones.Lenguaje);
                            }
                            else {
                        ...
                        if (opciones.Alert == null) {
                            SITRADOC.Alert(opciones.dialogAlert, opciones.Mensaje, '', opciones.Lenguaje);
                        }
                        else {
                            SITRADOC.Alert(opciones.Alert.titleMensaje, opciones.Alert.Mensaje, '', opciones.Lenguaje);
                        }
                    }
                }
            });
        ...
        var funccion = 'SITRADOC.LayoutBarra("' + pager.toString() + '")';
        ...
        if (!opciones.GridLocal) {
            ...
            $('#' + grilla).jqGrid({
                ...
                groupingView: {
                    ...
                    //  groupCollapse: true,
                    // groupText : ['<b>{0} - {1} Postor(s)</b>'],
                    groupText: ['<b style="background-color:#FFFFDC"> {0} - {1} Expediente(s) </b>'],
    ...
    LoadDropDownListItems: function (name, url, parameters, selected) {
        ...
        combo.disabled = true;
        var items = "";
        var resultado = SITRADOC.Ajax(url, parameters, false);
    ...
    CargarComboGrilla: function (url, parameters) {
        ...
        var cadena = "";
        var respuesta = SITRADOC.Ajax(url, parameters, false);
        if (respuesta != null) {
    ...
    InicializarMsg: function () {
        $(".msgModel").show();
        SITRADOC.msgHide();
        SITRADOC.ShowElement(".msgModel .loading");
    },
    ...
    msgConfirm: function (rpt, Message, funcionout) {
        ...
        if (rpt) {
            $(".msgModel").slideUp(200, function () {
                ...
                SITRADOC.ShowElement(".msgModel .mexito");
            ...
            $(".msgModel").slideUp(200, function () {
                ...
                SITRADOC.ShowElement(".msgModel .merror");
            });
        }
        $(".msgModel").show();
...
function InvocarVista(url, tipo, estdoc, idOficina) {
    ...
    var item =
   {
    ...
    var urlItem = url + "?parametro=" + JSON.stringify(item);

    jQuery("#divProceso").html('');
    jQuery("#divProceso").load(urlItem, function (responseText, textStatus, request) {
        $.validator.unobtrusive.parse('#divProceso');
        if (request.status != 200) return;
        $('#style_selector').append('');
        $('.divProceso').modal('show');
    });
}

function InvocarVistaFrai(url, tipo, estdoc, idOficina) {
...
function LlenarDropDowListAutomatico(control, url, MensajeError) {
    $.ajax({
        url: baseUrl + url,
        data: {},
        dataType: "json",
        type: "POST",
        error: function () {
            $('#' + control).html("<option value=''>--Seleccione--</option>");
            alert(MensajeError);
        },
