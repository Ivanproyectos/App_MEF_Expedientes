var Estado_grilla = 'Estado_grilla';
...
function Estado_ConfigurarGrilla() {
    ...
    var colModels = [
        { name: 'EDITAR', index: 'EDITAR', align: 'center', width: 60, hidden: false, sortable: false, formatter: Estado_actionEditar },
        { name: 'ELIMINAR', index: 'ELIMINAR', align: 'center', width: 70, hidden: false, sortable: false, formatter: Estado_actionEliminar },
        { name: 'ACTIVO', index: 'ACTIVO', align: 'center', width: 55, hidden: false, sortable: false, formatter: Estado_actionActivo },
    ...
    SICA.Grilla(Estado_grilla, Estado_barra, '', '400', '', "Lista de Estado", '', 'ID_DOMINIO', colNames, colModels, 'ID_DOMINIO', opciones);
}
...
function Estado_CargarGrilla() {
    ...
    var auditoria = Autorizacion.Ajax(url, item, false);
    jQuery("#" + Estado_grilla).jqGrid('clearGridData', true).trigger("reloadGrid");
    if (auditoria != null) {
        if (auditoria.EJECUCION_PROCEDIMIENTO) {
            $.each(auditoria.OBJETO, function (i, v) {
                var rowKey = jQuery("#" + Estado_grilla).getDataIDs();
                ...
                jQuery("#" + Estado_grilla).jqGrid('addRowData', v.ID_DOMINIO, myData);
            });
            jQuery("#" + Estado_grilla).trigger("reloadGrid");
            $('#Grilla_Load').hide();
        } else {
            jAlert(auditoria.MENSAJE_SALIDA, 'Atención');
            $('#Grilla_Load').hide();
...
function Estado_Registrar() {
    if ($("#frmMantenimientoEstado").valid()) {
        if ($('#AccionEstado').val() == "M") {
            Estado_Actualizar();
        } else {
            jConfirm("¿Desea guardar este registro ?", "Atención", function (r) {
                if (r) {
                    ...
                    var url = baseUrl + 'Administracion/Estado/Estado_Insertar';
                    var auditoria = Autorizacion.Ajax(url, item, false);
                    if (auditoria != null) {
                        if (auditoria.EJECUCION_PROCEDIMIENTO) {
                            if (!auditoria.RECHAZAR) {
...
function Estado_CambiarEstado(ID_DOMINIO, MiCheck) {
    var url = baseUrl + 'Administracion/Estado/Estado_Estado';
    var item = {
    ...
    if (auditoria != null && auditoria != "") {
        if (auditoria.EJECUCION_PROCEDIMIENTO) {
            if (!auditoria.RECHAZAR) {
                //$("#" + Estado_grilla).jqGrid('setCell', data.CODIGO, 'FLG_ESTADO', MiCheck.checked ? '1' : '0');
            } else {
                jAlert(auditoria.MENSAJE_SALIDA, 'Atención');
            }
        } else {
